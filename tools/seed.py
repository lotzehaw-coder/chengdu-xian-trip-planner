...
TRIP = {
    ...
    'about': 'Flights MH526 / MH527 are booked; exact times are on the e-ticket. Trains, vans and hotels are not booked yet and are marked tentative. Map opens the place in 高德地图 Amap, Google Maps or Apple Maps, in a new tab. Everything is saved in this browser only: no accounts, no server.',                                         # sanity box for coordinates (Sichuan + Shaanxi)            # day-trip towns, so map searches land in the right city
}
